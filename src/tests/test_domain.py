...
def test_enable_product_with_price_equal_or_less_than_zero():
    ...
    result = product.enable()
    assert result.message == "The price must be greater than zero to enable the product"
...
def test_disable_product_with_price_equal_zero():
    product = NewProduct.get_product(name="Hello", price=0)
    product.status = ENABLED
    result = product.disable()
    assert result is None
